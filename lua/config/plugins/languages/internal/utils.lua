...
function M.compile_typst(cmd_args)
    local filename = cmd_args.fargs[1]
    local outpath = cmd_args.fargs[2]
    ...
    local target_pdf = outpath
        or (vim.fn.fnamemodify(filename, ":p:r") .. ".pdf")
    outpath = vim.fn.fnamemodify(target_pdf, ":.")
