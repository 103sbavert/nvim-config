...
local function is_path_inside_dir(path, dir)
    ...
    if path_abs == nil or dir_abs == nil then return false end

    dir_abs = dir_abs:gsub('/+$', '')
    if path_abs == dir_abs then return true end
...
local function source_path_for_target(file)
    ...
    if not ok or type(source_paths) ~= 'table' then return nil end
...
local function get_chezmoi_source_dir()
    if cached_chezmoi_src_dir ~= nil and cached_chezmoi_src_dir ~= '' then return cached_chezmoi_src_dir end
    ...
    if type(source_root) ~= 'string' or source_root == '' then return nil end
...
local function is_chezmoi_source_file(file)
    ...
    if chezmoi_src_dir == nil or chezmoi_src_dir == '' then return false end

    return is_path_inside_dir(file, chezmoi_src_dir)
end

local function is_ignored_chezmoi_source(file_abs)
    if file_abs == nil or file_abs == '' then return false end
    ...
    for _, pattern in ipairs(patterns) do
        if type(pattern) == 'string' and pattern ~= '' then
            ...
            if anchored_pattern:sub(-1) ~= '$' then anchored_pattern = anchored_pattern .. '$' end

            if file_abs:match(anchored_pattern) then return true end
        end
    end
...
local function show_chezmoi_prompt(file, source)
    ...
    if choice == 2 then open_chezmoi_source(file, source) end
end

local function show_apply_prompt_for_source(source_abs)
    if source_abs == nil or source_abs == '' then return end
    ...
    commands.apply {
        ...
        on_stderr = function(_, data)
            if type(data) == 'string' and data ~= '' then vim.notify(data, vim.log.levels.WARN) end
        end,
    }
end
...
vim.api.nvim_create_autocmd('BufReadPost', {
    ...
    callback = function(args)
        ...
        if vim.b[buf].chezmoi_checked then return end
        ...
        vim.schedule(function()
            ...
            if vim.api.nvim_buf_get_name(buf) ~= file then return end

            if is_chezmoi_source_file(file) then return end

            local source = source_path_for_target(file)
            if source == nil then return end

            if source == file_abs then return end
...
vim.api.nvim_create_autocmd('BufWritePost', {
    ...
    callback = function(args)
        ...
        if not is_chezmoi_source_file(file_abs) then return end
    ...
    callback = function()
        ...
        local function edit_complete(arg_lead, _, _)
            ...
            for _, arg in ipairs { '--watch', '--force' } do
                if arg:find(arg_lead, 1, true) then table.insert(completions, arg) end
            end
            return completions
        end
